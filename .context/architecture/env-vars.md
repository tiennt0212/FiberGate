...
| `DOMAIN` | **Required** | (no usable default) | Public domain for nginx/certbot — needs DNS pointed at the host, ports 80/443/8228 open to the internet. Also the single source for what `fiber-node` announces to the Fiber network, so leaving it blank silently costs you every payment (see gotchas.md). |
| `FIBER_P2P_DOMAIN` | Optional | falls back to `DOMAIN` | A second, DNS-only hostname for the same host, used for `fiber-node`'s announced P2P/WSS address only. Exists for one situation: `DOMAIN` sitting behind a CDN proxy (Cloudflare's orange cloud), which forwards only HTTP/HTTPS ports and so drops 8228 at the edge — the node then announces a name nobody can dial while the dashboard on 443 keeps working. `certbot-init` adds this name to the certificate automatically, so `/wss` keeps validating. `create-fibergate` asks about this (only for a real domain, never for `localhost`) and writes the answer, so the blank is the tool's decision rather than the merchant's homework — empty is the normal, correct value. |
| `FIBER_ANNOUNCED_ADDRS` | Derived — **not** in `.env` | `/dns4/$FIBER_P2P_DOMAIN-or-$DOMAIN/tcp/8228` + the `/wss` form | Built in both compose files from `DOMAIN` and passed to `fnn` (which reads this name natively); it is not a merchant-facing setting. Deliberately has no `.env` entry — the announced address is always this deploy's own host, which `DOMAIN` already names, so a second setting could only ever disagree with the first. Both forms are announced at once: plain TCP for native peers, `/wss` for browser wallets (nginx serves both on `:8228`). Announcing anything else — a bare IP, an onion address — means editing the compose line directly. The `${DOMAIN:+...}` guard matters: a blank `DOMAIN` must yield an empty value, since `fnn` panics at boot on an unparseable multiaddr but tolerates an empty one. |
