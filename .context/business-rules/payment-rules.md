...
**BR-POL-001:** Phase 1: in-process interval worker chạy trong container `fibergate-core` mỗi 10 giây (không còn Vercel Cron — self-hosted chạy container dài hạn chứ không phải serverless). `/api/cron/poll-invoices` giữ lại như endpoint optional để trigger poll thủ công. **Phase 2 (issue #13, implemented + live-verified 2026-07-13)**: thêm WebSocket subscription `subscribe_store_changes` (RPC module `pubsub` của FNN, tồn tại từ bản stable v0.8.1) làm đường chính phát hiện thanh toán — xem chi tiết thiết kế + 2 gotcha phát hiện lúc verify live tại `architecture/system-design.md`. Interval poll (`lib/poller/worker.ts`) giảm tần suất còn **30s** làm fallback, vẫn giữ nguyên chứ không tắt hẳn, vì doc chính thức ghi rõ cơ chế này "primarily intended for Cross-Chain Hub integration rather than general client use".

**BR-POL-005:** Client subscribe `store_changes` phải lọc: chỉ xử lý variant `PutCkbInvoiceStatus`, và chỉ những `payment_hash` có tồn tại trong bảng `invoices` nội bộ — bỏ qua mọi variant/payment_hash khác trong stream. Implement tại `apps/web/lib/poller/invoice-listener.ts` (biến-filter) gọi vào `apps/web/lib/poller/invoice-poller.ts`'s `applyInvoiceStatusUpdate()` (payment_hash-lookup-filter, tái dùng chung `applyNodeStatus()`/`TERMINAL_TRANSITIONS` với poller RPC-driven batch — 1 nơi duy nhất quyết định node status nào là terminal).
